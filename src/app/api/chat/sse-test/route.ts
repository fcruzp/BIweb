 ...
 * This endpoint goes through the middleware (auth check) and uses the same
 * TransformStream + heartbeat pattern as the chat route.
 *
 * GET /api/chat/sse-test?prompt=Hello
 *
 * Use this to verify:
 * 1. SSE streaming works through Caddy reverse proxy
 * 2. Heartbeats are received by the client
 * 3. Z-AI works in an SSE context
 */
export async function GET(request: Request) {
  ...
  // Same TransformStream pattern as chat route
